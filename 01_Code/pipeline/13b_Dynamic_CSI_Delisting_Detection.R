#==============================================================================#
#==== 13b_Dynamic_CSI_Delisting_Detection.R ===================================#
#==== Delisting Detection Audit for Dynamic CSI Grid ===========================#
#==============================================================================#
#
# PURPOSE:
#   For each dynamic CSI C/M/T grid combination, measure how many confirmed CSI
#   firms subsequently delist and how many CRSP delistings were not preceded by
#   a confirmed CSI classification.
#
# DEFINITIONS:
#   - Headline detection uses confirmation_date <= dlstdt, because the firm is
#     classified as CSI only after the T-month confirmation window.
...
#
# OUTPUTS:
#   03_Data_Output/2_Robustness_Checks/Necessary/temporary_csi/tables/
...
cat("\n[13b_Dynamic_CSI_Delisting_Detection.R] START:",
    format(Sys.time()), "\n")
...
cat(sprintf("  Confirmed CSI event rows  : %d\n",
            nrow(events_grid[event_status == "confirmed_csi"])))
...
#==============================================================================#
# 2. Confirmed CSI events and event-level follow-up
#==============================================================================#
  ...
  event_hits_confirmation[
    ...
    .(
      ...
      months_confirmation_to_delisting =
        12L * (year(dlstdt) - year(confirmation_date)) +
        (month(dlstdt) - month(confirmation_date))
    )
  ],
...
firm_detail <- merge(
  ...
  firm_hits_confirmation[
    ...
    .(
      ...
      first_detecting_confirmation_date = confirmation_date
    )
  ],
...
bankruptcy_hits_confirmation <- merge(
  ...
  by = "permno",
  allow.cartesian = TRUE
)[confirmation_date <= bankruptcy_date]
...
md_lines <- c(
  ...
  "firm has a confirmed CSI classification before or on the delisting date ",
  "(`confirmation_date <= dlstdt`). This is stricter than the permanent-CSI ",
  ...
  "",
  paste0("- Confirmed CSI event rows: ",
         fn_fmt_int(base_row$confirmed_csi_event_rows), "."),
  paste0("- Confirmed CSI firms: ",
  ...
  paste0("- Adverse-delisting firms missed after confirmation: ",
         ...
         "."),
  paste0("- Confirmed CSI firms that later had an adverse delisting: ",
         fn_fmt_pct(
           base_row$confirmed_csi_firm_adverse_delisting_followup_rate_pct
         ),
         "."),
  paste0("- Confirmed CSI event rows followed by adverse delisting: ",
         fn_fmt_int(
