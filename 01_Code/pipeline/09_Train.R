...
#
# LABEL:
#   y is already paper-aligned by 05C:
#   event in calendar year t + 1 -> annual row y_{i,t}.
#   09_Train.R therefore uses y directly and does not shift labels again.
#
#==============================================================================#
...
MODELS_TO_RUN <- strsplit(
  Sys.getenv("MODELS_TO_RUN", "fund,raw,raw_plus_latent,latent_raw"),
  ",",
  fixed = TRUE
)[[1]]
MODELS_TO_RUN <- trimws(MODELS_TO_RUN)
MODELS_TO_RUN <- MODELS_TO_RUN[nzchar(MODELS_TO_RUN)]
LOAD_EXISTING_COMPARISON <- fn_env_flag("TRAIN_LOAD_EXISTING_COMPARISON", default = FALSE)
...
FILE_REQUIREMENTS <- list(
  ...
  raw_plus_latent = PATH_FEATURES_RAW_PLUS_LATENT,
  latent_raw      = PATH_FEATURES_LATENT_RAW,
  latent_fund     = PATH_FEATURES_LATENT_FUND
)

PREREQ_MESSAGES <- list(
  fund            = "Run 06B_FeatureEngineering.R first.",
  raw             = "Run 06B_FeatureEngineering.R first.",
  raw_plus_latent = "Run 08B_Autoencoder.py with VAE_INPUT='raw' first.",
...
if ("raw_plus_latent" %in% MODELS_TO_RUN) {
  features_raw_plus_latent <- as.data.table(
    arrow::read_parquet(PATH_FEATURES_RAW_PLUS_LATENT))
  cat(sprintf("  features_raw+latent  : %d rows, %d cols\n",
              nrow(features_raw_plus_latent), ncol(features_raw_plus_latent)))
}
...
ID_COLS <- c("permno", "year", "y", "censored", "param_id",
             "response_track", "gvkey", "datadate", "lifetime_years",
             ...
             "months_observed", "tier1_window_complete",
...
if ("raw_plus_latent" %in% MODELS_TO_RUN) {
  RAW_PLUS_LATENT_COLS <- setdiff(
    names(features_raw_plus_latent)[sapply(features_raw_plus_latent, is.numeric)],
    ID_COLS)
  cat(sprintf("  raw+latent features  : %d columns\n",
              length(RAW_PLUS_LATENT_COLS)))
}
...
fn_train_xgboost <- function(
  ...
  for (k in seq_along(splits$oot$cv_folds)) {
    ...
    train_pos_k <- panel_to_pos[train_labelled] + 1L
    val_pos_k   <- panel_to_pos[val_labelled]   + 1L
    xgb_folds[[length(xgb_folds)+1L]] <- val_pos_k - 1L
    cv_fold_specs[[length(cv_fold_specs)+1L]] <- list(
      fold_id   = k,
      train_pos = train_pos_k,
      val_pos   = val_pos_k
    )
  ...
  ## 5G. CV validation predictions
  cat("  Scoring CV validation folds...\n")
  cv_fold_preds <- data.table()
  cv_fold_metrics <- data.frame()
  
  if (length(cv_fold_specs) > 0L) {
    cv_fold_preds <- data.table::rbindlist(lapply(cv_fold_specs, function(fold) {
      dtrain_k <- xgb.DMatrix(
        data  = X_train_qt[fold$train_pos, , drop = FALSE],
        label = y_train[fold$train_pos]
      )
      dval_k <- xgb.DMatrix(
        data  = X_train_qt[fold$val_pos, , drop = FALSE],
        label = y_train[fold$val_pos]
      )
      model_k <- xgb.train(
        params  = final_params,
        data    = dtrain_k,
      ...
      data.table(
        ...
        permno  = train_full$permno[fold$val_pos],
        year    = train_full$year[fold$val_pos],
        y       = y_train[fold$val_pos],
        p_csi   = predict(model_k, dval_k)
      )
    }), use.names = TRUE)
    
    cv_fold_metrics <- data.table::rbindlist(lapply(
      split(cv_fold_preds, cv_fold_preds$fold_id),
      function(dt) {
        out <- fn_eval_metrics(
          y_true = dt$y,
          y_pred = dt$p_csi,
          feature_set_name = feature_set_name,
          set_name = sprintf("cv_fold_%s", unique(dt$fold_id)[1])
        )
        out$fold_id <- unique(dt$fold_id)[1]
        out
      }
    ), fill = TRUE)
    ...
    metrics_cv <- data.frame(
      ...
      auc_roc       = round(mean(cv_fold_metrics$auc_roc,       na.rm = TRUE), 4),
      avg_precision = round(mean(cv_fold_metrics$avg_precision, na.rm = TRUE), 4),
      recall_fpr3   = round(mean(cv_fold_metrics$recall_fpr3,   na.rm = TRUE), 4),
      recall_fpr5   = round(mean(cv_fold_metrics$recall_fpr5,   na.rm = TRUE), 4),
      brier         = round(mean(cv_fold_metrics$brier,         na.rm = TRUE), 4),
      stringsAsFactors = FALSE
    )
  } else {
    metrics_cv <- data.frame(
  ...
  cat(sprintf("\n  [%s] CV AP: %.4f | CV AUC: %.4f | Test AP: %.4f | AUC: %.4f | R@FPR5: %.4f\n",
              toupper(feature_set_name), metrics_cv$avg_precision,
              metrics_cv$auc_roc, te$avg_precision, te$auc_roc, te$recall_fpr5))
...
if ("raw" %in% MODELS_TO_RUN) {
  ...
  saveRDS(result_raw, file.path(DIR_TABLES_XGB_TRACK, "xgb_raw.rds"))
...
if ("fund" %in% MODELS_TO_RUN) {
  ...
  saveRDS(result_fund, file.path(DIR_TABLES_XGB_TRACK, "xgb_fund.rds"))
  results_list[["fund"]] <- result_fund
  cat("[09_Train.R] xgb_fund.rds saved.\n")
}

if ("raw_plus_latent" %in% MODELS_TO_RUN) {
  cat("\n[09_Train.R] Raw + latent_raw features\n")
  result_raw_plus_latent <- fn_train_xgboost(
    "raw_plus_latent",
    copy(features_raw_plus_latent),
    RAW_PLUS_LATENT_COLS,
    splits
  )
  saveRDS(result_raw_plus_latent,
          file.path(DIR_TABLES_XGB_TRACK, "xgb_raw_plus_latent.rds"))
  results_list[["raw_plus_latent"]] <- result_raw_plus_latent
  cat("[09_Train.R] xgb_raw_plus_latent.rds saved.\n")
}
...
if ("latent_fund" %in% MODELS_TO_RUN) {
  ...
  saveRDS(result_latent_fund, file.path(DIR_TABLES_XGB_TRACK, "xgb_latent_fund.rds"))
...
if ("latent_raw" %in% MODELS_TO_RUN) {
  ...
  saveRDS(result_latent_raw, file.path(DIR_TABLES_XGB_TRACK, "xgb_latent_raw.rds"))
...
SAVED_FILES <- list(
  fund            = file.path(DIR_TABLES_XGB_TRACK, "xgb_fund.rds"),
  raw             = file.path(DIR_TABLES_XGB_TRACK, "xgb_raw.rds"),
  raw_plus_latent = file.path(DIR_TABLES_XGB_TRACK, "xgb_raw_plus_latent.rds"),
  latent_raw      = file.path(DIR_TABLES_XGB_TRACK, "xgb_latent_raw.rds"),
  latent_fund     = file.path(DIR_TABLES_XGB_TRACK, "xgb_latent_fund.rds")
)

for (nm in names(SAVED_FILES)) {
  if (LOAD_EXISTING_COMPARISON && !nm %in% MODELS_TO_RUN && file.exists(SAVED_FILES[[nm]])) {
...
dir.create(file.path(DIR_TABLES_TRACK, "09_train"), recursive = TRUE, showWarnings = FALSE)
write.csv(
  eval_combined,
  file.path(DIR_TABLES_TRACK, "09_train", "xgb_eval_table.csv"),
  row.names = FALSE
)
cat("[09_Train.R] evaluation_results.rds and xgb_eval_table.csv saved.\n")
...
MODEL_LABELS <- c(
  ...
  latent_raw = "latent-only",
  latent_fund = "lat-fund"
)

for (nm in c("fund", "raw", "raw_plus_latent", "latent_raw", "latent_fund")) {
  if (!nm %in% names(results_list)) next
  res <- results_list[[nm]]
  cv  <- res$eval_table[res$eval_table$set == "cv_expanding_window",]
  te  <- res$eval_table[res$eval_table$set == "test",]
  cat(sprintf("  %-14s | %.4f   | %.4f   | %.4f   | %.4f | %.4f\n",
              MODEL_LABELS[nm], cv$avg_precision,
