#==============================================================================#
#==== 11D_IndexConstruction_Optimal03b.R ======================================#
#==== Optimal 03b ag_raw CSI Overlay on CRSP-like Market Indices ==============#
#==============================================================================#
#
# PURPOSE:
...
#     and AutoGluon ag_raw CSI predictions from 03b_Output.
...
#
# OUTPUTS:
#   DIR_TABLES/<RESPONSE_TRACK>/<INDEX_RUN_TAG>/
...
cat("\n[11D_IndexConstruction_Optimal03b.R] START:", format(Sys.time()), "\n")

`%||%` <- function(a, b) if (is.null(a) || length(a) == 0L || all(is.na(a))) b else a

RUN_STARTED <- Sys.time()
SCRIPT_PATH <- normalizePath("11D_IndexConstruction_Optimal03b.R", mustWork = FALSE)

## NOTE: The 03_Output / 03b_Output split was retired in the AgonyAndExcstasy
## layout — there is a single 03_Data_Output tree now. The revised 03b-style
## predictions are the only ones written to disk. The original assertion that
## DIR_OUTPUT ended in "03b_Output" is removed.
RUN_TAG <- Sys.getenv(
  "INDEX_RUN_TAG",
  unset = sprintf("11d_index_optimal03b_ag_raw_%s", format(RUN_STARTED, "%Y%m%d_%H%M%S"))
)
OUT_DIR <- file.path(DIR_TABLES_TRACK, RUN_TAG)
if (dir.exists(OUT_DIR) && length(list.files(OUT_DIR, all.files = FALSE, no.. = TRUE)) > 0L) {
  stop(sprintf("Refusing to overwrite non-empty output directory: %s", OUT_DIR))
}
...
MODEL_KEY <- "raw"
MODEL_LABEL <- "AutoGluon raw (03b optimal input)"
THRESHOLD_METHODS <- c(
  fpr1 = "CV FPR <= 1%",
  fpr3 = "CV FPR <= 3%",
  youden = "CV Youden J"
...
cat("[11D] Loading CRSP-like constituents and monthly returns...\n")
...
cat("[11D] Loading 03b ag_raw predictions and estimating CV thresholds...\n")
...
cat("[11D] Building benchmark and model-filtered quarterly weights...\n")
...
cat("[11D] Computing monthly drifted portfolio returns...\n")
...
cat("[11D] Computing performance metrics...\n")
...
cat("[11D] Computing exclusion summary...\n")
...
cat("[11D] Computing FP/FN/TP/TN error-cost decomposition...\n")
...
cat("\n[11D] OOS performance by index and best Sharpe model-filtered strategy:\n")
