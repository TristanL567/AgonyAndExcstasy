...
fn_n_unique <- function(x) {
  length(unique(x[!is.na(x)]))
}

n_rows   <- nrow(features)
n_permno <- fn_n_unique(features$permno)
...
fn_oos_diag <- function(idx, label) {
  ...
  n_csi     <- sum(y_vec == 1L, na.rm = TRUE)
  n_obs     <- sum(!is.na(y_vec))
  n_firms   <- fn_n_unique(perm_vec)
  csi_firms <- fn_n_unique(perm_vec[!is.na(y_vec) & y_vec == 1L])
  data.frame(
    split         = label,
    n_rows        = length(idx),
    n_firms       = n_firms,
...
splits <- list(
  ...
  meta = list(
    ...
    features_path    = PATH_FEATURES_RAW,
    train_end        = TRAIN_END,
...
cat(sprintf("[08_Split.R] splits.rds saved: %s\n", PATH_SPLITS))
...
fn_write_parquet <- function(df, path) {
  if (requireNamespace("arrow", quietly = TRUE)) {
    arrow::write_parquet(df, path)
    return(invisible(path))
  }

  tmp_csv <- paste0(path, ".csv.tmp")
  tmp_py <- paste0(path, ".py.tmp")
  ...
  status <- system2("python", c(tmp_py, tmp_csv, path))
  unlink(tmp_csv)
  unlink(tmp_py)
...
fn_write_parquet(
  ...
  file.path(DIR_FEATURES_TRACK, "split_labels_oot.parquet")
)
...
fn_write_parquet(
  ...
  file.path(DIR_FEATURES_TRACK, "split_labels_oos.parquet")
)

cat("[08_Split.R] Parquets saved for Python.\n")
cat(sprintf("  split_labels_oot.parquet : %s\n",
            file.path(DIR_FEATURES_TRACK, "split_labels_oot.parquet")))
cat(sprintf("  split_labels_oos.parquet : %s\n",
            file.path(DIR_FEATURES_TRACK, "split_labels_oos.parquet")))
...
## Feature split-prevalence figure is descriptive — route under 1_Descriptive_Statistics
split_prev_dir <- file.path(DIR_DESC_TRACK, "06_features")
dir.create(split_prev_dir, recursive = TRUE, showWarnings = FALSE)
ggsave(
  file.path(split_prev_dir, "split_prevalence.png"),
