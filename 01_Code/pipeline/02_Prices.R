...
##---------------------------------------------------------------------------##
## 2B. Shumway-style delisting return imputation
##
##   Shumway (1997, J. Business) documents that CRSP's dlret is missing for a
##   substantial fraction of performance delistings, biasing terminal returns
##   toward zero. The standard correction substitutes -0.55 for missing dlret
##   on adverse delistings (NASDAQ-style delistings being the harder case;
##   for parsimony we apply a single value across all adverse codes, per
##   PCL_DLRET_IMPUTE_VALUE in config.R).
##
##   Codes that trigger imputation:
##     PCL_DLRET_IMPUTE_TRIGGER_CODES = c(400:490, 500:599)
##       400-490 : liquidations
##       500-599 : dropped by exchange (performance, regulatory, dropped-from-NASDAQ)
##
##   Mergers (200-299) and voluntary delistings retain dlret as-is.
##---------------------------------------------------------------------------##

n_missing_pre <- sum(
  delisting$dlstcd %in% PCL_DLRET_IMPUTE_TRIGGER_CODES &
    is.na(delisting$dlret),
  na.rm = TRUE
)
...
delisting <- delisting |>
  mutate(
    ...
    dlret = if_else(dlret_imputed, PCL_DLRET_IMPUTE_VALUE, dlret)
  )

cat(sprintf(
  "[02_Prices.R] Shumway imputation: %d adverse-delisting rows had dlret = NA -> set to %.2f\n",
  n_missing_pre, PCL_DLRET_IMPUTE_VALUE
))
...
prices_daily_clean <- prices_daily_raw |>
  ...
  left_join(
    delisting |>
      ...
      select(permno, ym, dlstdt, dlret, dlstcd),
    by = c("permno", "ym")
  ) |>
  mutate(
    dlret = if_else(date == dlstdt, dlret, NA_real_),
