...
OUTPUTS per model:
  Tables/AutoGluon/<RESPONSE_TRACK>/ag_{MODEL}/shap_values.parquet
  Tables/AutoGluon/<RESPONSE_TRACK>/ag_{MODEL}/shap_meta.parquet
  Tables/AutoGluon/<RESPONSE_TRACK>/ag_{MODEL}/shap_importance.parquet
  Tables/AutoGluon/<RESPONSE_TRACK>/ag_{MODEL}/pdp_1d.parquet
  Tables/AutoGluon/<RESPONSE_TRACK>/ag_{MODEL}/pdp_2d.parquet
...
DIR_OUTPUT = DIR_DATA_OUTPUT  # back-compat alias
...
DIR_MODELS = DIR_MODELLING_TRACK / "AutoGluon"
DIR_TABLES = DIR_MODELLING_TRACK / "AutoGluon"
...
print(f"\n[10B] ══════════════════════════════════════")
print(f"  MODEL    : {MODEL}")
print(f"  RESPONSE : {RESPONSE_TRACK}")
...
if not IS_BUCKET:
    ...
    df = df[(df["split"] == "test") & df[LABEL_COL].notna()].copy()
...
ID_COLS = ["permno", "year", "y", "censored", "param_id", "gvkey", "datadate",
           ...
           "vae_split", "split_oot", "y_loser", "y_structural",
           "y_dynamic_csi", "y_permanent_csi", "fwd_cagr",
