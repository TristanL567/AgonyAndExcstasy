...
══ MODEL MATRIX ═══════════════════════════════════════════════════════════════
  ...
  Track 1 — Dynamic CSI (paper-style event-year target; no label shift in 09C)
  ...
  Track 3 — Permanent CSI (permanent capital loss; no label shift)
...
══ KEY FIXES vs v5 ════════════════════════════════════════════════════════════
  ...
  1. CSI annual target:
       The label pipeline performs paper-style alignment upstream:
       confirmed trigger in year t+1 -> y_{i,t}=1. 09C does not shift y.
...
if _env_root:
    print(f"[09C] MT_ROOT provided: {_env_root}")
...
def resolve_output_dir():
    # MT_OUTPUT_DIR env var is now obsolete (single output tree); if set to
    # an absolute path, treat as an override of DIR_DATA_OUTPUT.
...
print(f"[09C] Output folder: {DIR_OUTPUT}")
...
def read_response_track():
    ...
    if env_track:
        return env_track
    config_path = DATA_ROOT / "01_Code" / "pipeline" / "config.R"
...
assert RESPONSE_TRACK in LABEL_TRACKS, \
    f"RESPONSE_TRACK must be one of {sorted(LABEL_TRACKS)}; got {RESPONSE_TRACK}"
TRACK_FOLDER = _track_folder(RESPONSE_TRACK)

DIR_FEATURES = DIR_PR_NEC / TRACK_FOLDER / "Features"
DIR_LABELS   = DIR_PR_NEC / TRACK_FOLDER / "Labels"
...
PATH_LABELS_MODEL_READY = DIR_LABELS  / "labels_model_ready.rds"
PATH_LABELS_BUCKET      = DIR_LABELS_LEGACY_FLAT / "labels_bucket.rds"
PATH_LABELS_STRUCTURAL  = DIR_LABELS_LEGACY_FLAT / "labels_structural.rds"
...
SEED          = 123
TIME_LIMIT    = int(os.environ.get("AG_TIME_LIMIT", "1800"))
PRESET        = os.environ.get("AG_PRESET", "good_quality")
CV_TIME_LIMIT = int(os.environ.get("AG_CV_TIME_LIMIT", "900"))
CV_PRESET     = os.environ.get("AG_CV_PRESET", "medium_quality")
...
def _detect_cuda_gpus():
    requested = os.environ.get("AG_NUM_GPUS", "auto").strip().lower()
    if requested != "auto":
...
def _parse_excluded_models():
    raw = os.environ.get("AG_EXCLUDED_MODEL_TYPES", "").strip()
    if not raw:
        return []
    return [x.strip() for x in raw.split(",") if x.strip()]

AG_NUM_CPUS             = _env_int_or_none("AG_NUM_CPUS")
AG_NUM_GPUS             = _detect_cuda_gpus()
AG_NUM_BAG_FOLDS        = int(os.environ.get("AG_NUM_BAG_FOLDS", "0"))
AG_NUM_STACK_LEVELS     = int(os.environ.get("AG_NUM_STACK_LEVELS", "0"))
AG_CV_NUM_BAG_FOLDS     = int(os.environ.get("AG_CV_NUM_BAG_FOLDS",
                                             str(AG_NUM_BAG_FOLDS)))
AG_CV_NUM_STACK_LEVELS  = int(os.environ.get("AG_CV_NUM_STACK_LEVELS",
                                             str(AG_NUM_STACK_LEVELS)))
AG_EXCLUDED_MODEL_TYPES = _parse_excluded_models()
AG_REFIT_FULL           = os.environ.get("AG_REFIT_FULL", "0").strip() == "1"
...
MODEL_CONFIG = {
    ...
    "raw_plus_latent": {
        "description": "Raw engineered features plus raw-VAE latent features",
        "path"       : DIR_FEATURES / "features_raw_plus_latent.parquet",
        "loader"     : "parquet",
        "label_col"  : LABEL_CSI,
        "is_bucket"  : False,
    ...
    "structural": {
        ...
        "is_bucket"  : True,
        "prereq"     : "Run 06_Merge.R and 06B_FeatureEngineering.R for the permanent track first.",
    },
    "structural_latent_fund": {
        ...
        "label_col"  : LABEL_STRUCTURAL,
        "is_bucket"  : True,
        "prereq"     : "Run 06_Merge/06B and 08B with VAE_INPUT='fund' for the permanent track first.",
    },
    "structural_raw": {
    ...
    "structural_latent_raw": {
        ...
        "label_col"  : LABEL_STRUCTURAL,
        "is_bucket"  : True,
        "prereq"     : "Run 06_Merge/06B and 08B with VAE_INPUT='raw' for the permanent track first.",
    },
}
...
TRACK_LABEL = (
    "Temporary/Tewari CSI" if RESPONSE_TRACK == "dynamic_csi"
    else "Permanent CSI"
)
...
print(f"\n[09C] {'='*54}")
print(f"  MODEL        : {MODEL}")
print(f"  RESPONSE_TRACK: {RESPONSE_TRACK}")
print(f"  Description  : {cfg['description']}")
print(f"  Features     : {cfg['path'].name}")
print(f"  Label        : {LABEL_COL}")
print(f"  Track        : {TRACK_LABEL if not IS_BUCKET else 'Legacy bucket/structural'}")
print(f"  Output       : {DIR_TABLES_RUN}")
print(f"  Preset/time  : {PRESET} / {TIME_LIMIT}s "
      f"(CV {CV_PRESET} / {CV_TIME_LIMIT}s)")
print(f"  Resources    : cpus={AG_NUM_CPUS or 'auto'} | gpus={AG_NUM_GPUS}")
print(f"  Bag/stack    : stage1={AG_NUM_BAG_FOLDS}/{AG_NUM_STACK_LEVELS} | "
      f"cv={AG_CV_NUM_BAG_FOLDS}/{AG_CV_NUM_STACK_LEVELS}")
print(f"  Excluded     : {AG_EXCLUDED_MODEL_TYPES or 'none'}")
print(f"  Refit full   : {AG_REFIT_FULL}")
print(f"[09C] {'='*54}\n")
...
ID_COLS = [
    ...
    "y_loser", "y_structural", "y_dynamic_csi", "y_permanent_csi",
    "fwd_cagr", "n_months", "bucket",
    "event_date", "confirmation_date", "event_year", "label_year",
    "dynamic_event_date", "dynamic_confirmation_date",
    "dynamic_event_year", "dynamic_label_year", "dynamic_label_censored",
    "permanent_event_date", "permanent_confirmation_date",
    "permanent_event_year", "permanent_label_year", "permanent_label_censored",
...
# ==============================================================================
# 7A. Dynamic CSI models — merge OOT split labels, no label shift
# ==============================================================================
...
if not IS_BUCKET:
    ...
    ## y is already the paper-style annual target:
    ## confirmed trigger in year t+1 -> feature row year t receives y=1.
    df_with_label = df[df[LABEL_COL].notna()].copy()
    df_with_label[LABEL_COL] = df_with_label[LABEL_COL].astype(int)
    df_eval = df_with_label.copy()

    print(f"\n[09C] Dynamic CSI annual target loaded:")
    print(f"  Rows with y            : {len(df_with_label):,}")
    print(f"  y prevalence           : {df_eval[LABEL_COL].mean():.4f}")
    ...
    print(f"  Train : {len(train_all):,} / {len(train_eval):,}  "
          f"prev={train_eval[LABEL_COL].mean():.4f}")
    print(f"  Test  : {len(test_all):,} / {len(test_eval):,}  "
          f"prev={test_eval[LABEL_COL].mean():.4f}")
...
#
#   Holdout = 2011-2015. Labels are already aligned by 06_Merge.
...
## Labels are already aligned by 06_Merge, so fold fitting uses the full
## training span available up to each fold boundary.
FOLD_BOUNDARIES = [
    (2, 2001, 2002, 2006),
    (3, 2006, 2007, 2010),
...
if ag_test_data is not None:
    print(f"  Test rows     : {len(ag_test_data):,}  (eval-safe {TEST_START_YEAR}-{TEST_END_YEAR})")
...
fit_kwargs = dict(
    ...
    refit_full       = AG_REFIT_FULL,
    set_best_to_refit_full = AG_REFIT_FULL,
)
if AG_NUM_CPUS is not None:
    fit_kwargs["num_cpus"] = AG_NUM_CPUS
if AG_NUM_GPUS > 0:
    fit_kwargs["num_gpus"] = AG_NUM_GPUS
if AG_EXCLUDED_MODEL_TYPES:
    fit_kwargs["excluded_model_types"] = AG_EXCLUDED_MODEL_TYPES
if IS_BUCKET:
...
RUN_FEATURE_IMPORTANCE = os.environ.get("AG_FEATURE_IMPORTANCE", "0") == "1"
if RUN_FEATURE_IMPORTANCE:
    ...
    print("\n[09C] Feature importance skipped (set AG_FEATURE_IMPORTANCE=1 to enable).")
...
#
#   Test metrics (2016-2019) = model selection.
...
print(f"  (Test {TEST_START_YEAR}-{TEST_END_YEAR} = model selection basis)")
print(f"  (OOS 2020–2024 = index construction, not model selection)\n")

test_label = f"test_{TEST_START_YEAR}_{TEST_END_YEAR}"
...
for fid, tend, vst, vend in FOLD_BOUNDARIES:
    ...
    fold_fit_kw = dict(
        ...
        num_bag_folds    = AG_CV_NUM_BAG_FOLDS,
        num_stack_levels = AG_CV_NUM_STACK_LEVELS,
        refit_full       = False,
        set_best_to_refit_full = False,
    )
    if AG_NUM_CPUS is not None:
        fold_fit_kw["num_cpus"] = AG_NUM_CPUS
    if AG_NUM_GPUS > 0:
        fold_fit_kw["num_gpus"] = AG_NUM_GPUS
    if AG_EXCLUDED_MODEL_TYPES:
        fold_fit_kw["excluded_model_types"] = AG_EXCLUDED_MODEL_TYPES
    if IS_BUCKET:
...
eval_summary = {
    ...
    "num_bag_folds"      : AG_NUM_BAG_FOLDS,
    "num_stack_levels"   : AG_NUM_STACK_LEVELS,
    "cv_num_bag_folds"   : AG_CV_NUM_BAG_FOLDS,
    "cv_num_stack_levels": AG_CV_NUM_STACK_LEVELS,
    "excluded_model_types": AG_EXCLUDED_MODEL_TYPES,
    "label_col"          : LABEL_COL,
    "is_bucket"          : IS_BUCKET,
    "label_shift"        : "none - labels are forward-aligned in 06_Merge",
    "eval_period"        : f"{TEST_START_YEAR}-{TEST_END_YEAR}",
