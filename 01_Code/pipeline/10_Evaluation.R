...
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/ag_preds_test_eval.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/ag_preds_oos.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/ag_eval_summary.json
#
# INPUTS (per model, from 10B):
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/shap_values.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/shap_meta.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/shap_importance.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/pdp_1d.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/pdp_2d.parquet
#   DIR_TABLES_AUTOGLUON_TRACK/ag_{MODEL}/shap_waterfall_firm.parquet
#
# OUTPUTS:
#   Tables:
...
dir.create(DIR_TABLES_TRACK, recursive = TRUE, showWarnings = FALSE)
...
for (m in MODELS) {
  key       <- m$key
  test_path <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "ag_preds_test_eval.parquet")
  oos_path  <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
  ...
  PREDS[[key]] <- list(
    test = fn_read_parquet(test_path),
    oos  = if (file.exists(oos_path))
      fn_read_parquet(oos_path) else NULL
  )
...
saveRDS(eval_perf, file.path(DIR_TABLES_EVAL_TRACK, "eval_performance_all.rds"))
...
saveRDS(eval_year, file.path(DIR_TABLES_EVAL_TRACK, "eval_by_year_all.rds"))
...
for (key in names(PREDS)) {
  shap_path <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "shap_values.parquet")
  meta_path <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "shap_meta.parquet")
  imp_path  <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "shap_importance.parquet")
  wf_path   <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "shap_waterfall_firm.parquet")
  ...
  shap_dt <- fn_read_parquet(shap_path)
  meta_dt <- fn_read_parquet(meta_path)
  imp_dt  <- fn_read_parquet(imp_path)
...
for (key in names(PREDS)) {
  pdp_path  <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "pdp_1d.parquet")
  pdp2_path <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", key),
                         "pdp_2d.parquet")
  ...
  pdp_dt  <- fn_read_parquet(pdp_path)
  fig_dir <- FIGS$models[[tolower(MODEL_SHORTS[[key]])]]
  short   <- MODEL_SHORTS[[key]]
  ...
  if (file.exists(pdp2_path)) {
    pdp2_dt <- fn_read_parquet(pdp2_path)
...
if (length(imp_available) >= 2) {
  ...
  for (k in names(imp_available)) {
    pdp_f <- file.path(DIR_TABLES_AUTOGLUON_TRACK, paste0("ag_", k),
                       "pdp_1d.parquet")
    if (!file.exists(pdp_f)) next
    sub <- fn_read_parquet(pdp_f)[feature == key_feat]
    if (nrow(sub) == 0) next
    sub[, model_key   := k]
...
saveRDS(eval_thresh, file.path(DIR_TABLES_EVAL_TRACK, "eval_threshold_all.rds"))
