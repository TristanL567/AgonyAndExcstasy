...
# AgonyAndExcstasy layout: single output tree (no more 03_Output/03b_Output split).
...
DIR_FEATURES_ROOT = DIR_PR_NEC          # parent of {track}/Features/
DIR_LABELS        = DIR_PR_NEC          # parent of {track}/Labels/

VALID_RESPONSE_TRACKS = {"dynamic_csi", "permanent_csi"}


def _track_folder(rt: str) -> str:
...
def env_flag(name: str, default: bool = True) -> bool:
    ...
    if value is None:
        ...
        if config_path.exists():
            ...
            match = re.search(rf"{name}\s*<-\s*(TRUE|FALSE)", text)
            if match:
                value = match.group(1)
    if value is None:
        return default
...
assert RESPONSE_TRACK in VALID_RESPONSE_TRACKS, (
    f"Unknown RESPONSE_TRACK '{RESPONSE_TRACK}'. "
    f"Choose one of: {sorted(VALID_RESPONSE_TRACKS)}"
)
TRACK_FOLDER = _track_folder(RESPONSE_TRACK)

CSI_USE_TERMINAL_FAILURE_INDICATORS = env_flag(
    "CSI_USE_TERMINAL_FAILURE_INDICATORS",
    default=True,
)
# MT_OUTPUT_DIR env var is now obsolete (single output tree) but kept for
# back-compat: if set to an absolute path, it overrides DIR_DATA_OUTPUT.
_output_override = os.environ.get("MT_OUTPUT_DIR")
if _output_override:
    _output_path = Path(_output_override)
    if _output_path.is_absolute():
        DIR_DATA_OUTPUT = _output_path

# Modelling output (track-aware) under 03_Data_Output/3_Modelling_Results/
DIR_MODELLING_TRACK = DIR_DATA_OUTPUT / "3_Modelling_Results" / "Necessary" / TRACK_FOLDER
DIR_TABLES   = DIR_MODELLING_TRACK / "AutoGluon"        # written by 09C, read here for diagnostics
DIR_FEATURES = DIR_FEATURES_ROOT / TRACK_FOLDER / "Features"
DIR_MODELS   = DIR_MODELLING_TRACK / "VAE"
DIR_VAE_FIGS = DIR_MODELLING_TRACK / "figures" / "08_autoencoder"
DIR_FIGURES  = DIR_MODELLING_TRACK / "figures"

DIR_FEATURES.mkdir(parents=True, exist_ok=True)
DIR_VAE_FIGS.mkdir(parents=True, exist_ok=True)
DIR_MODELS.mkdir(parents=True,  exist_ok=True)

print(f"[08B] RESPONSE_TRACK: {RESPONSE_TRACK}")
print(f"[08B] TERMINAL CSI : {CSI_USE_TERMINAL_FAILURE_INDICATORS}")
print(f"[08B] Output folder: {DIR_DATA_OUTPUT}")
print(f"[08B] Feature folder: {DIR_FEATURES}")
...
VAE_INPUT = os.environ.get("VAE_INPUT", "raw").strip().lower()
assert VAE_INPUT in {"fund", "raw"}, "VAE_INPUT must be 'fund' or 'raw'"
...
if VAE_INPUT == "raw":
    print(f"  Raw+latent   : {PATH_FEATURES_RAW_PLUS_LATENT.name}")
...
ID_COLS = [
    ...
    "eval_split", "split_oot", "vae_split",
...
LABEL_META_PREFIXES = ("y_", "dynamic_", "permanent_", "pcl_")
LABEL_META_SUFFIXES = ("_label_year", "_label_censored", "_event_year")
BINARY_COLS = ["recession"]

feature_cols = [
    c for c in df.columns
    if c not in ID_COLS
    and not c.startswith(LABEL_META_PREFIXES)
    and not c.endswith(LABEL_META_SUFFIXES)
...
def main():
    ...
    cfg_save = {
        **CFG,
        ...
        "raw_plus_latent_file": (
            str(PATH_FEATURES_RAW_PLUS_LATENT)
            if VAE_INPUT == "raw" else None
        ),
    }
    ...
    plot_training_curves(
        history,
        DIR_VAE_FIGS / f"vae_training_curves_{fig_suffix}.png"
    )
    plot_latent_space(
        z_train, y_train_full,
        DIR_VAE_FIGS / f"vae_latent_space_{fig_suffix}.png"
    )
    ...
    if VAE_INPUT == "raw":
        latent_for_join = features_latent[
            ["permno", "year", *latent_cols, "split"]
        ].copy()
        raw_plus_latent = features_input.merge(
            latent_for_join,
            on=["permno", "year"],
            how="inner",
            validate="one_to_one",
        )
        assert len(raw_plus_latent) == len(features_latent), (
            "raw+latent row count mismatch after merge"
        )
        assert raw_plus_latent[latent_cols].isna().sum().sum() == 0, (
            "NAs in raw+latent latent columns"
        )
        raw_plus_latent.to_parquet(PATH_FEATURES_RAW_PLUS_LATENT, index=False)
        print(f"[08B] Saved raw+latent: {PATH_FEATURES_RAW_PLUS_LATENT}")
