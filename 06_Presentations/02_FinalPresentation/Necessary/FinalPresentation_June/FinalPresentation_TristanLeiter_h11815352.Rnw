...
\vspace{0.1em}
\begin{block}{Read this as robustness, not replacement}
\scriptsize
Temporary-CSI sensitivity supports the main run as a defensible, non-outlier setting. Completed C/M/T runs remain positive across all four universes; the main run is below median in Total/Large and above median in Mid/Small.
\end{block}
\vspace{-0.15em}
\begin{alertblock}{Limit}
\scriptsize
This sensitivity evidence is temporary CSI only. Three blocked partial C/M/T configurations are excluded, so the result supports stability of completed runs rather than an exhaustive permanent-CSI claim.
\end{alertblock}
\end{column}
\end{columns}
