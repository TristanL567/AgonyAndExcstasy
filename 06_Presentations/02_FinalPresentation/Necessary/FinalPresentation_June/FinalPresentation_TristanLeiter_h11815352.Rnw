...
\textbf{Robustness read}
\vspace{-0.15em}
\begin{itemize}
  \item 24 complete/reused temporary-CSI C/M/T runs are available.
  \item The accepted main run is inside the completed-run distribution, not an outlier.
  \item Main-run alpha is below the median for Total/Large and above the median for Mid/Small.
  \item Sensitivity changes magnitude more than direction across universes.
