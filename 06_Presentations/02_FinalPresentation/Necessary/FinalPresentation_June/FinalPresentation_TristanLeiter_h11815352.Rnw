...
\begin{tabular}{L{1.45cm}L{1.45cm}R{0.62cm}R{0.62cm}R{0.68cm}R{0.82cm}}
\toprule
Objective & Config & OOS AP & OOS AUC & R@FPR3 & 11C TM alpha \\
\midrule
Composite & \texttt{C090\_M000\_T012} & 0.557 & 0.920 & 0.407 & +0.169pp \\
AP winner & \texttt{C060\_M000\_T012} & 0.566 & 0.864 & 0.204 & +0.052pp \\
11C TM & \texttt{C090\_M020\_T018} & 0.274 & 0.911 & 0.316 & +0.226pp \\
Baseline & \texttt{C080\_M020\_T018} & 0.308 & 0.896 & 0.255 & +0.065pp \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.05em}
...
\end{alertblock}
\scriptsize
Use sensitivity as robustness evidence, not as a replacement for the final index suite. The accepted-label temporary index grid is separately complete for Youden/FPR1/FPR3/FPR5, lockouts 1/2/3/5, universes, and 0/5/10/20 bps.
\remarkfoot{Sources: local \texttt{sensitivity\_cmt\_model\_summary.csv}; \texttt{sensitivity\_cmt\_index\_summary.csv}; \texttt{temporary\_presentation\_key\_findings.csv}. TM alpha = total-market annualized return difference vs benchmark.}
\end{frame}
...
\begin{frame}{Error-Cost Decomposition}
\fontsize{5.15pt}{5.75pt}\selectfont
\textbf{OOS error-cost rows decompose the selected best strategy by confusion category}
\vspace{-0.35em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.8pt}
\begin{tabular}{L{0.78cm}L{1.95cm}R{0.78cm}R{0.78cm}R{0.78cm}R{0.78cm}R{0.78cm}}
\toprule
\multicolumn{7}{l}{\textbf{Temporary CSI}}\\
\midrule
...
\begin{tabular}{L{0.78cm}L{1.95cm}R{0.78cm}R{0.78cm}R{0.78cm}R{0.78cm}R{0.78cm}}
\toprule
\multicolumn{7}{l}{\textbf{Permanent CSI}}\\
\midrule
Universe & Best rule & FP cost & FN cost & TP gain & TN gain & Net \\
\midrule
...
\vspace{-0.15em}
\scriptsize
False positives are opportunity-cost exclusions; true negatives preserve benchmark exposure. Net is the source OOS strategy difference versus the matched benchmark, while category signs are retained from the decomposition output.
\remarkfoot{Source: model-specific \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv}; matched on track, universe, model, threshold, lockout/rule, strategy id, and OOS period.}
\end{frame}
