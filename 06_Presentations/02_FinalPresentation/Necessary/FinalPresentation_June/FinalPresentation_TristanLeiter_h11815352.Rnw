...
\fontsize{5.65pt}{6.2pt}\selectfont
\textbf{OOS non-additive contribution diagnostic for the selected temporary-CSI best strategies}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.65pt}
\begin{tabular}{L{0.78cm}L{2.15cm}R{0.80cm}R{0.80cm}R{0.80cm}R{0.80cm}R{0.80cm}}
\toprule
Universe & Best rule & FP cost & FN cost & TP gain & TN gain & Geo alpha \\
\midrule
...
\end{alertblock}
\scriptsize
FP/FN/TP/TN are annualized category diagnostics from the source decomposition and are not additive under geometric compounding. Geo alpha is the exact selected OOS strategy geometric return minus the matched benchmark return before cost overlays.
\remarkfoot{Source: model-specific \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv}; matched to \texttt{best\_by\_track\_index\_cost.csv} by track, universe, model, threshold, lockout, strategy id, and OOS period.}
\end{frame}
...
\fontsize{5.65pt}{6.2pt}\selectfont
\textbf{OOS non-additive contribution diagnostic for the selected permanent-CSI best strategies}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.65pt}
...
\scriptsize
FP/FN/TP/TN are annualized category diagnostics from the source decomposition and are not additive under geometric compounding. Geo alpha is the exact selected OOS strategy geometric return minus the matched benchmark return before cost overlays.
\remarkfoot{Source: model-specific \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv}; matched to \texttt{best\_by\_track\_index\_cost.csv} by track, universe, model, threshold, permanent rule, strategy id, and OOS period.}
\end{frame}
