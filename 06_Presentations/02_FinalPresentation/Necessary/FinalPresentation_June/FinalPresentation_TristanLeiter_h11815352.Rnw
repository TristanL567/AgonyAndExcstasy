...
\scriptsize
Terminal-failure route enabled. The temporary-CSI label has 8,517 positive annual cells in the full observable scaffold. Unresolved end-of-sample labels remain in the canonical panel, but are excluded from the labelled sample shown below.
\end{alertblock}
...
\scriptsize
Prevalence is computed on labelled cells only: $y=1/(y=0+y=1)$. Unresolved labels are retained in canonical artifacts for scaffold consistency, but excluded from supervised training, label-based evaluation, and this displayed count table.
\end{block}
\end{frame}
...
\scriptsize
For both tracks, labelled rows are $y=0+y=1$ and prevalence is $y=1/(y=0+y=1)$. Unresolved rows remain in the cleaned panel but are excluded from supervised training, evaluation metrics, and this displayed count table.
\end{block}
\end{frame}
...
\scriptsize
The temporary-CSI C/M/T sensitivity grid is available locally. All 27 run IDs are represented; 24 have complete/reused compact results and 3 are explicitly marked \texttt{blocked\_partial}. Derived presentation summaries remain local-only under \texttt{03\_Data\_Output}.
\end{alertblock}
...
\end{block}
\remarkfoot{Sources: local \texttt{presentation\_ready/sensitivity\_cmt\_model\_summary.csv}; \texttt{temporary\_blocked\_config\_disclosure.csv}; AE-SENS-PRES-003 evidence.}
\end{frame}
...
\fontsize{5.25pt}{5.85pt}\selectfont
\textbf{Sensitivity winners point to different objectives, not one universal setting}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.15pt}
\begin{tabular}{L{1.25cm}L{1.35cm}R{0.54cm}R{0.54cm}R{0.57cm}R{0.58cm}R{0.58cm}R{0.62cm}}
\toprule
Objective & Config & CV AP & CV AUC & CV FPR3 & Test AP & Test AUC & Test FPR3 \\
\midrule
Composite & \texttt{C090\_M000\_T012} & 0.442 & 0.916 & 0.370 & 0.438 & 0.917 & 0.347 \\
AP winner & \texttt{C060\_M000\_T012} & \textbf{0.473} & 0.825 & 0.171 & \textbf{0.476} & 0.859 & 0.173 \\
11C TM & \texttt{C090\_M020\_T018} & 0.198 & 0.897 & 0.279 & 0.209 & 0.906 & 0.271 \\
Baseline & \texttt{C080\_M020\_T018} & 0.203 & 0.862 & 0.230 & 0.199 & 0.877 & 0.200 \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.05em}
...
\end{alertblock}
\scriptsize
Use sensitivity as robustness evidence, not as a replacement for the final index suite. OOS index-alpha comparisons are shown later in the index and robustness sections.
\remarkfoot{Sources: local \texttt{sensitivity\_cmt\_model\_summary.csv}; \texttt{temporary\_presentation\_key\_findings.csv}. FPR3 columns are R@FPR3.}
\end{frame}
...
\scriptsize
Temporary CSI remains highly rankable, but AP and fixed-FPR recall are the decision metrics because positives are rare. \textbf{AG Exp. Dataset + VAE} is strongest on train/CV and OOS AP/FPR recall; \textbf{AG Expanded Dataset} remains the strongest 2016--2019 test benchmark and has marginally higher OOS AUC.
\end{alertblock}
...
\begin{alertblock}{Main read}
\scriptsize
Permanent CSI is harder: positives are rarer and OOS prevalence is much lower. \textbf{AG Exp. Dataset + VAE} is the best non-raw CV/test challenger, while \textbf{AG Latent Dataset (VAE)} is the strongest OOS robustness signal.
\end{alertblock}
...
\begin{frame}{Test-Set Index Check: 2016--2019}
\fontsize{4.95pt}{5.45pt}\selectfont
\textbf{Isolated test-set-only rows; best 10 bps strategies within raw and raw+latent package}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.15pt}
\begin{tabular}{L{0.70cm}L{0.74cm}L{2.05cm}R{0.64cm}R{0.64cm}R{0.62cm}R{0.58cm}R{0.62cm}}
\toprule
Track & Univ. & Test winner & Bench. & Strategy & Alpha & Sharpe & Turnover \\
\midrule
Temp. & Total & raw+latent; Youden/3yr & 13.75\% & 14.15\% & +0.41pp & 0.946 & 8.9\% \\
Temp. & Large & raw+latent; Youden/3yr & 14.17\% & 14.41\% & +0.24pp & 0.984 & 11.9\% \\
Temp. & Mid & raw; Youden/5yr & 13.73\% & 14.06\% & +0.33pp & 0.905 & 80.9\% \\
Temp. & Small & raw+latent; Youden/2yr & 11.37\% & 12.26\% & +0.89pp & 0.670 & 63.0\% \\
Perm. & Total & raw+latent; Youden/perm. & 13.75\% & 14.19\% & +0.45pp & 1.002 & 8.4\% \\
Perm. & Large & raw+latent; Youden/perm. & 14.17\% & 14.43\% & +0.26pp & 1.041 & 10.9\% \\
Perm. & Mid & raw+latent; Youden/perm. & 13.73\% & 14.16\% & +0.43pp & 0.946 & 78.2\% \\
Perm. & Small & raw+latent; Youden/perm. & 11.37\% & 12.20\% & +0.83pp & 0.677 & 62.3\% \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.2em}
\begin{alertblock}{How to read this versus OOS}
\scriptsize
These rows are a 2016--2019 test-set check, not the 2020+ OOS index construction used in the following result slides. The isolated package contains only \texttt{period = test} performance rows and validation checks report zero OOS rows.
\end{alertblock}
...
\begin{frame}{Index Results: Temporary CSI at 0 bps}
\fontsize{4.85pt}{5.35pt}\selectfont
\textbf{OOS benchmark and best strategy by universe; no transaction-cost overlay}
\vspace{-0.25em}
\begin{table}[ht]
\centering
...
\end{alertblock}
\remarkfoot{Source: \texttt{best\_by\_track\_index\_cost.csv}; Ann. SD and ES 2.5\% from model-specific \texttt{index\_performance\_gross\_and\_net\_by\_tc.csv}.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Temporary CSI: Error-Cost Diagnostic}
\fontsize{4.95pt}{5.45pt}\selectfont
\textbf{OOS contribution diagnostic with geometric attribution adjustment}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.15pt}
\begin{tabular}{L{0.74cm}L{1.82cm}R{0.64cm}R{0.64cm}R{0.64cm}R{0.64cm}R{0.67cm}R{0.70cm}}
\toprule
Universe & Best rule & FP cost & FN cost & TP gain & TN gain & Geo adj. & Geo alpha \\
\midrule
Total & AG Base, Youden 3yr & -2.03pp & +0.00pp & +0.08pp & +1.79pp & +0.59pp & +0.43pp \\
Large & AG Base, Youden 3yr & -1.50pp & +0.00pp & +0.00pp & +1.24pp & +0.41pp & +0.15pp \\
Mid & AG Base, FPR5 5yr & +0.05pp & +0.00pp & +0.00pp & +0.27pp & +0.19pp & +0.51pp \\
Small & AG +VAE, Youden 3yr & -4.00pp & -0.02pp & +0.42pp & +3.97pp & +0.27pp & +0.64pp \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.15em}
\begin{alertblock}{Why temporary CSI outperforms}
\scriptsize
...
\end{alertblock}
\scriptsize
Geo adj. is the geometric compounding/weight-interaction adjustment: Geo alpha minus the four annualized FP/FN/TP/TN diagnostics. Geo alpha is selected OOS strategy geometric return minus matched benchmark return before cost overlays.
\remarkfoot{Sources: model-specific \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv}; \texttt{best\_by\_track\_index\_cost.csv}; AE-PRES-FINAL-REV2-009 decomposition audit.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Temporary CSI: Realized Active Attribution}
\fontsize{4.95pt}{5.45pt}\selectfont
\textbf{Selected OOS strategies at 10 bps; attribution reconciles to realized alpha}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.15pt}
...
\toprule
Universe & Strategy & TP excl. & FP excl. & Retained & TC & Geo adj. & Alpha \\
\midrule
...
\end{table}
\vspace{-0.15em}
\begin{alertblock}{What changes relative to the diagnostic slide}
\scriptsize
TP/FP are direct exclusion effects. FN/TN are not direct realized-alpha columns; retained names affect alpha through the retained-stock reweighting term. Each row satisfies TP excl. + FP excl. + retained + TC + geo adj. = realized alpha.
\end{alertblock}
\scriptsize
At 10 bps, temporary CSI alpha is mostly the net of false-positive opportunity cost and retained-stock reweighting, with Small Cap receiving the clearest direct TP exclusion gain.
\remarkfoot{Source: \texttt{AE-ATTRIB-001\_config\_level\_attribution.csv}; selected OOS rows, \texttt{transaction\_cost\_bps=10}; validation in \texttt{AE-ATTRIB-001\_reconciliation\_checks.csv}.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Index Results: Temporary CSI at 10 bps}
\fontsize{4.85pt}{5.35pt}\selectfont
\textbf{OOS benchmark and best strategy by universe; 10 bps cost rows}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.25pt}
...
\end{alertblock}
\remarkfoot{Strategy rows use source-provided 10 bps net performance; expected annual drag is approximately annualized gross turnover $\times$ 0.001. Source: \texttt{best\_by\_track\_index\_cost.csv}.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Index Results: Permanent CSI at 0 bps}
\fontsize{4.85pt}{5.35pt}\selectfont
\textbf{OOS benchmark and best strategy by universe; permanent-removal rule}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.25pt}
...
\end{alertblock}
\remarkfoot{Benchmark rows are model-key \texttt{bench}, strategy \texttt{bench\_mw}, period \texttt{oos}. Source: model-specific \texttt{index\_performance\_gross\_and\_net\_by\_tc.csv}.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Permanent CSI: Error-Cost Diagnostic}
\fontsize{4.95pt}{5.45pt}\selectfont
\textbf{OOS contribution diagnostic with geometric attribution adjustment}
\vspace{-0.25em}
\begin{table}[ht]
...
Universe & Best rule & FP cost & FN cost & TP gain & TN gain & Geo adj. & Geo alpha \\
\midrule
Total & AG +VAE, FPR5 perm. & -0.49pp & +0.00pp & +0.01pp & +0.52pp & +0.23pp & +0.27pp \\
Large & AG +VAE, FPR5 perm. & -0.26pp & +0.00pp & +0.00pp & +0.30pp & +0.19pp & +0.23pp \\
Mid & AG Base, FPR5 perm. & -0.55pp & +0.00pp & +0.00pp & +0.90pp & +0.39pp & +0.74pp \\
Small & AG Latent, FPR3 perm. & -0.74pp & +0.00pp & +0.04pp & +1.03pp & -0.01pp & +0.32pp \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.15em}
\begin{alertblock}{Why permanent CSI outperforms}
\scriptsize
...
\begin{frame}{Permanent CSI: Realized Active Attribution}
\fontsize{4.95pt}{5.45pt}\selectfont
\textbf{Selected OOS strategies at 10 bps; attribution reconciles to realized alpha}
\vspace{-0.25em}
\begin{table}[ht]
...
Universe & Strategy & TP excl. & FP excl. & Retained & TC & Geo adj. & Alpha \\
\midrule
Total & AG +VAE, FPR5 perm. & +0.01pp & -0.49pp & +0.52pp & -0.01pp & +0.23pp & +0.26pp \\
Large & AG +VAE, FPR5 perm. & +0.00pp & -0.26pp & +0.30pp & -0.01pp & +0.19pp & +0.22pp \\
...
\begin{alertblock}{What changes relative to the diagnostic slide}
\scriptsize
The realized view separates direct exclusion effects from retained-stock reweighting. Permanent CSI has small TP exclusion gains because positives are rare; retained-stock reweighting and controlled FP cost drive most realized alpha.
\end{alertblock}
\scriptsize
FN/TN remain diagnostics for missed or correctly retained names; they enter realized alpha only through active retained-stock weights after portfolio renormalization.
\remarkfoot{Source: \texttt{AE-ATTRIB-001\_config\_level\_attribution.csv}; selected OOS rows, \texttt{transaction\_cost\_bps=10}; validation in \texttt{AE-ATTRIB-001\_reconciliation\_checks.csv}.}
\end{frame}

%==== 4b. Data findings =======================================================%

\begin{frame}{Index Results: Permanent CSI at 10 bps}
\fontsize{4.85pt}{5.35pt}\selectfont
\textbf{OOS benchmark and best strategy by universe; 10 bps cost rows}
\vspace{-0.25em}
\begin{table}[ht]
...
\end{alertblock}
\remarkfoot{Strategy rows use source-provided 10 bps net performance; expected annual drag is approximately annualized gross turnover $\times$ 0.001. Delta pp compares against the unchanged benchmark row.}
\end{frame}
...
\textbf{Winner ranking is stable from 0 to 20 bps; costs apply to gross buy+sell turnover}
\vspace{-0.25em}
\begin{table}[ht]
\centering
...
\fontsize{5.25pt}{5.85pt}\selectfont
\textbf{Best strategies keep their ranking at 10 bps; turnover is gross buy+sell traded weight}
\vspace{-0.35em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.8pt}
\begin{tabular}{L{0.78cm}L{2.35cm}R{0.95cm}R{0.95cm}R{0.95cm}R{0.95cm}}
\toprule
\multicolumn{6}{l}{\textbf{Temporary CSI}}\\
\midrule
...
\begin{tabular}{L{0.78cm}L{2.35cm}R{0.95cm}R{0.95cm}R{0.95cm}R{0.95cm}}
\toprule
\multicolumn{6}{l}{\textbf{Permanent CSI}}\\
\midrule
Universe & Best strategy & Ann. turnover & Geo ret. 0 bps & Geo ret. 10 bps & Delta from TC \\
\midrule
...
\vspace{-0.15em}
\scriptsize
Mid and Small Cap exclusions create the largest annualized gross turnover, measured as buy plus sell traded weight, so cost drag is visibly larger there. The 0--20 bps sweep did not change any top winner ranking.
\remarkfoot{Source: \texttt{turnover\_summary.csv}, \texttt{winner\_turnover\_summary\_20bps.csv}, and \texttt{best\_by\_track\_index\_cost.csv}.}
\end{frame}
...
\begin{frame}{Sensitivity: Main Run Versus C/M/T Grid}
...
\textbf{Key results: best alpha distribution}
\vspace{-0.35em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.4pt}
...
\vspace{0.15em}
\begin{itemize}
  \item The accepted main run sits inside the completed-run distribution.
  \item Main-run alpha is below the median for Total/Large and above the median for Mid/Small.
  \item Sensitivity changes magnitude more than direction: completed runs remain positive across all four universes.
  \item Three blocked partial C/M/T configs remain excluded from completed-run conclusions.
\end{itemize}
\end{column}
\end{columns}
...
\scriptsize
The grid represents 27 C/M/T configurations: 24 are complete or reused, and three are \texttt{blocked\_partial}: \texttt{C080\_M000\_T012}, \texttt{C080\_M000\_T018}, and \texttt{C060\_M020\_T028}. Blocked cases are represented in manifests but excluded from completed-run conclusions.
\end{block}
