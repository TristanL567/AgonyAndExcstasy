...
\begin{itemize}
  \item Models: \texttt{raw} (AG Expanded Dataset), \texttt{fund} (AG Base Dataset), \texttt{latent\_raw} (AG Latent Dataset), \texttt{raw\_plus\_latent} (AG Exp. Dataset + VAE).
...
\begin{itemize}
  ...
  \item Error-cost decomposition: selected \texttt{fund}, \texttt{raw\_plus\_latent}, and \texttt{latent\_raw} \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv} files mapped exactly in \texttt{SLIDE\_DATA\_SOURCES.md}.
\end{itemize}
\vspace{0.3em}
