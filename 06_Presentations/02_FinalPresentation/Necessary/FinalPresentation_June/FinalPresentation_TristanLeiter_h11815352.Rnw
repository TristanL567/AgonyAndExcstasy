...
Universe & Strategy & Geo ret. & Ann. SD & Sharpe Ratio & Max DD & ES 2.5\% & Delta pp \\
\midrule
Total & Benchmark & 13.28\% & 19.03\% & 0.597 & -25.04\% & -11.62\% & 0.00pp \\
Total & AG Base Dataset; Youden/3yr & 13.71\% & 18.28\% & 0.635 & -23.79\% & -11.22\% & +0.43pp \\
Large & Benchmark & 14.26\% & 18.49\% & 0.656 & -24.79\% & -10.84\% & 0.00pp \\
Large & AG Base Dataset; Youden/3yr & 14.42\% & 17.98\% & 0.677 & -23.90\% & -10.65\% & +0.15pp \\
Mid & Benchmark & 9.54\% & 20.72\% & 0.401 & -24.58\% & -13.83\% & 0.00pp \\
Mid & AG Base Dataset; FPR5/5yr & 10.05\% & 20.70\% & 0.424 & -24.46\% & -13.85\% & +0.51pp \\
Small & Benchmark & 7.57\% & 23.80\% & 0.302 & -29.05\% & -16.12\% & 0.00pp \\
Small & AG Exp. Dataset + VAE; Youden/3yr & 8.20\% & 22.64\% & 0.332 & -29.33\% & -15.77\% & +0.63pp \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.15em}
...
\scriptsize
The 10 bps overlay leaves the temporary CSI winners unchanged. The source benchmark rows remain the unfiltered market-cap reference for each universe, reported at the matching cost row.
\end{alertblock}
\remarkfoot{Cost rows are source-provided net performance rows; cost overlay applies through the 11C transaction-cost contract. Source: \texttt{best\_by\_track\_index\_cost.csv}.}
\end{frame}
...
\midrule
Total & Benchmark & 13.28\% & 19.03\% & 0.597 & -25.04\% & -11.62\% & 0.00pp \\
Total & AG Exp. Dataset + VAE; FPR5/perm. & 13.55\% & 18.82\% & 0.614 & -24.72\% & -11.45\% & +0.27pp \\
Large & Benchmark & 14.26\% & 18.49\% & 0.656 & -24.79\% & -10.84\% & 0.00pp \\
Large & AG Exp. Dataset + VAE; FPR5/perm. & 14.49\% & 18.38\% & 0.670 & -24.51\% & -10.74\% & +0.23pp \\
Mid & Benchmark & 9.54\% & 20.72\% & 0.401 & -24.58\% & -13.83\% & 0.00pp \\
Mid & AG Base Dataset; FPR5/perm. & 10.28\% & 20.58\% & 0.436 & -24.52\% & -13.90\% & +0.74pp \\
Small & Benchmark & 7.57\% & 23.80\% & 0.302 & -29.05\% & -16.12\% & 0.00pp \\
Small & AG Latent Dataset (VAE); FPR3/perm. & 7.90\% & 23.78\% & 0.315 & -29.26\% & -16.35\% & +0.32pp \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.15em}
...
\scriptsize
The 10 bps overlay leaves the permanent CSI winners unchanged. Permanent CSI gains come from fixed-FPR rules rather than Youden-style broad exclusions.
\end{alertblock}
\remarkfoot{Geo ret., Ann. SD, Sharpe Ratio, Max DD, and ES 2.5\% are net OOS values from source performance rows. Delta pp is zero for benchmarks.}
\end{frame}
...
\textbf{Best strategies keep their ranking at 10 bps, but Mid and Small Cap absorb more cost drag}
\vspace{-0.35em}
\begin{table}[ht]
\centering
...
\vspace{-0.15em}
\scriptsize
Mid and Small Cap exclusions create the largest annualized gross turnover, so cost drag is visibly larger there. The 0--20 bps sweep did not change any top winner ranking.
...
\end{block}
\remarkfoot{Sources: \texttt{05\_comparisons/full\_grid\_best\_configs\_by\_objective.csv}; \texttt{05\_comparisons/full\_grid\_*}; \texttt{99\_reports/sensitivity\_results\_readme.md}.}
\end{frame}
