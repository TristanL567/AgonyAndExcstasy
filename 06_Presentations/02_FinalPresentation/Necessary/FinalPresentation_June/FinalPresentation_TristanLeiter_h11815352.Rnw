...
\scriptsize
Terminal-failure route enabled. The temporary-CSI label has 8,517 positive annual cells in the full observable scaffold. The 8,674 OOS $y=\mathrm{NA}$ rows are unresolved labels: 3,342 censored trigger labels plus 5,332 observable 2024 rows that would require unavailable 2025 event-year labels.
\end{alertblock}
...
\scriptsize
Prevalence is computed on labelled cells only: $y=1/(y=0+y=1)$. The retained $y=\mathrm{NA}$ rows are observable firm-years with unresolved labels, not dropped rows; they are excluded from supervised training and label-based evaluation.
\end{block}
\end{frame}
...
\begin{frame}{Appendix A19: Model AP Versus Index Alpha}
\fontsize{6.0pt}{6.6pt}\selectfont
\begin{columns}[T,onlytextwidth]
\begin{column}{0.66\textwidth}
\centering
\includegraphics[width=\linewidth]{../../../../05_Documentation/09_Epics/AE-SENS-CHART_Sensitivity_Index_Charts/charts/chart2_model_vs_index_sensitivity_scatter_refined.png}
\end{column}
\begin{column}{0.32\textwidth}
\textbf{Model versus portfolio objective}
\vspace{-0.15em}
\begin{itemize}
  \item AP and index alpha are related robustness diagnostics, but they are not the same objective.
  \item The AP winner and total-market alpha winner differ across C/M/T settings.
  \item The continuity baseline remains defensible, but it is not top-ranked.
  \item This supports presenting sensitivity as a robustness check rather than a replacement for the accepted main run.
\end{itemize}
\end{column}
\end{columns}
\remarkfoot{Sources: \texttt{chart2\_model\_vs\_index\_sensitivity\_scatter\_refined.png}; local \texttt{sensitivity\_cmt\_model\_summary.csv}; \texttt{sensitivity\_cmt\_index\_summary.csv}; AE-SENS-CHART-001R interpretation.}
\end{frame}
