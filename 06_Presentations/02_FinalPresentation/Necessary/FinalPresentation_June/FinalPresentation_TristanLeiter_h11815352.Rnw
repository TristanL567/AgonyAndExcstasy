...
\textbf{OOS contribution diagnostic reconciled to selected strategy geometric alpha}
\vspace{-0.25em}
\begin{table}[ht]
\centering
\setlength{\tabcolsep}{1.15pt}
...
\toprule
Universe & Best rule & FP cost & FN cost & TP gain & TN gain & Recon. & Geo alpha \\
\midrule
...
\end{alertblock}
\scriptsize
Recon. is the row-level reconciliation term that makes the displayed diagnostics sum to Geo alpha after rounding. Geo alpha is selected OOS strategy geometric return minus matched benchmark return before cost overlays.
\remarkfoot{Sources: model-specific \texttt{error\_cost\_decomposition\_by\_crsp\_universe.csv}; \texttt{best\_by\_track\_index\_cost.csv}; AE-PRES-FINAL-REV2-002 reconciliation evidence.}
\end{frame}
