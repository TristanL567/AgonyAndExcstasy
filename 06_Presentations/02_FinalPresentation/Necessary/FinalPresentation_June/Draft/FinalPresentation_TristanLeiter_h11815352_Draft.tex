...
\scriptsize
\textbf{Temporary CSI, \agexp{}.} The exclusion threshold is calibrated on cross-validation, then carried \emph{unchanged} to the held-out 2016--2019 test set.
\vspace{0.15em}
\begin{center}
...
\begin{itemize}\setlength\itemsep{0.18em}
  ...
  \item \textbf{100\%} of false positives fall inside the true-positive score range.
  \item Only \textbf{10.8\%} sit just above $\tau$ (within $[\tau,\tau+0.01]$).
\end{itemize}
\end{column}
\end{columns}

\vspace{0.05em}
\begin{block}{Read carefully}
\scriptsize
The false-positive rate \emph{drifts} from $3.0\%$ on CV to $8.7\%$ on test because $\tau$ is fixed before the test period is seen --- this is out-of-sample threshold drift, not the modelling table's ``Test-FPR3'', which re-fixes $3\%$ FPR on the test set itself.
\end{block}

\vspace{-0.15em}
\begin{alertblock}{What this means for the index}
\scriptsize
Every false positive overlaps the true-positive scores, so a higher threshold cannot remove them without dropping real CSI firms. This is the statistical ceiling; because CSI firms are only a small cap-weighted exposure, it becomes the index-level dilution ceiling.
\end{alertblock}
...
\scriptsize
The calibrated score-to-weight pipeline runs independently inside each universe, so every screened index is judged against its matching unfiltered benchmark.
\end{alertblock}
...
\begin{itemize}
  ...
  \item Large/Mid/Small split by cap rank \textcolor{darkgrey}{(insert breakpoints)}.
  \item Quarterly rebalancing; cap weights.
\end{itemize}
\end{column}
\begin{column}{0.49\textwidth}
\textbf{Comparators}
\scriptsize
\begin{itemize}
  \item Primary: the unfiltered cap-weighted benchmark, like-for-like per universe.
  \item Min-volatility and quality screens (main RQ): planned extension --- see limitations.
\end{itemize}
\end{column}
\end{columns}
...
\scriptsize
The crash-filter is useful, but not because it cleanly identifies only future CSI firms. The previous slide shows that thresholding is noisy; the index result shows what remains after that noise: modest positive alpha from reweighting away from flagged names, strongest where cap-weight dilution is lower. The thesis claim is therefore a bounded risk-screening and portfolio-composition effect, not a pure crash-avoidance alpha engine.
\end{alertblock}
...
\scriptsize
The risk-filter is most visible outside large caps: alpha rises from $+0.14$pp (Large) to $+0.39$pp (Mid) to $+0.55$pp (Small). This matches the dilution story: in smaller universes, flagged firms receive more portfolio weight and exclusions move more capital (about 85--106\% turnover vs.\ 10--13\% for the total market). Drawdown improves everywhere except for small caps.
\end{block}
\end{frame}
...
\scriptsize
The modelling section explains why the attribution looks like this: the score ranks risk, but any usable cutoff also removes many non-CSI firms. Direct TP avoidance is small, FP opportunity cost is large, and the remaining positive alpha comes from retained-name reweighting and geometric effects. This supports a risk-screening interpretation, not a pure event-avoidance claim.
\end{alertblock}
...
%==== LOW-VOLATILITY COMPARISON APPENDIX =====================================%
\begin{frame}{Identical universes, windows and costs isolate what the method adds}
\scriptsize
\begin{columns}[T,onlytextwidth]
\begin{column}{0.52\textwidth}
\begin{block}{Building the low-volatility comparator}
\scriptsize
\begin{itemize}\setlength\itemsep{0.4pt}
  \item Rank each firm by trailing realized volatility.
  \item Sort into quintiles Q1 (lowest) through Q5 (highest).
  \item Low-volatility holds Q1; Q5 is the high-volatility tail.
  \item Quintiles are rebuilt in each evaluation window.
\end{itemize}
\end{block}
\vspace{0.3mm}
{\scriptsize\color{MainColour} Realized volatility rises across quintiles (test)}\\[0.3mm]
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[font=\scriptsize,MainColour] at (0.50,-0.45) {held};
  \node[font=\scriptsize,warningRed] at (4.30,-0.45) {high-vol tail};
\end{tikzpicture}
\end{column}
\begin{column}{0.46\textwidth}
\begin{block}{Matched to the CSI results}
\scriptsize
\begin{itemize}\setlength\itemsep{0.4pt}
  \item Same four universes: Total Market, Large, Mid, and Small.
  \item Test: 2016-01-29 to 2019-12-31.
  \item OOS: 2020-01-31 to 2024-12-31.
  \item Benchmark at 0 bps; LowVol Q1 and CSI at 20 bps.
\end{itemize}
\end{block}
\vspace{0.3mm}
\begin{alertblock}{Comparison question}
\footnotesize
Does the ML crash filter add something beyond a transparent volatility screen, and is the difference meaningful once dilution and transaction costs are included?
\end{alertblock}
\end{column}
\end{columns}
\sourcefoot{\texttt{AE-ALPHA\_Risk-Return\_and\_Overlap\_Summary.md}; low-volatility quintile and CSI comparison outputs.}
\end{frame}

\begin{frame}{Low-vol's edge is large but regime-bound; CSI's is small but stable}
\scriptsize
\centering
{\scriptsize
  \tikz\fill[darkgrey](0,0) rectangle (0.24,0.24);~Naive Benchmark\quad
  \tikz{\fill[lightblue](0,0) rectangle (0.24,0.24);\draw[MainColour,line width=0.6pt](0,0) rectangle (0.24,0.24);}~LowVol Q1\quad
  \tikz\fill[MainColour](0,0) rectangle (0.24,0.24);~CSI dynamic\par}
\vspace{0.7mm}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.5\textwidth}
\centering
{\footnotesize\color{MainColour} Sharpe ratio --- higher is better}\\[0.7mm]
\begin{tikzpicture}[x=1cm,y=1cm]
...
\begin{column}{0.5\textwidth}
\centering
{\footnotesize\color{MainColour} Max drawdown $|\%|$ --- smaller is better}\\[0.7mm]
\begin{tikzpicture}[x=1cm,y=1cm]
  \draw[darkgrey,line width=0.6pt] (0.05,0) -- (3.95,0);
...
\end{columns}
\vspace{0.5mm}
\begin{alertblock}{Defensive trade-off}
\footnotesize
LowVol Q1 has the strongest test-window risk reduction, but OOS it gives up $-3.4$ pp of geometric return versus the benchmark. CSI keeps return close to benchmark ($+0.3$ pp OOS) while delivering smaller, steadier risk improvements.
\end{alertblock}
\sourcefoot{Average across four universes. Benchmark at 0 bps; LowVol Q1 and CSI at 20 bps.}
\end{frame}

\begin{frame}{CSI trims the tail low-vol targets --- yet it is not just a volatility sort}
\scriptsize
\begin{columns}[T,onlytextwidth]
\begin{column}{0.5\textwidth}
\centering
{\footnotesize\color{MainColour} Where dynamic CSI exclusions fall}\\
{\scriptsize\color{darkgrey} Share of excluded names by volatility quintile (test)}\\[0.7mm]
\begin{tikzpicture}[x=1cm,y=1cm]
  \draw[darkgrey,line width=0.6pt] (0,0) -- (5.05,0);
...
\end{tikzpicture}\\[0.7mm]
{\setbeamercolor{block title}{bg=posgreenL,fg=white}
\begin{block}{Retained portfolio remains diversified}
\scriptsize
Dynamic CSI retains about 35\% in Q1 and keeps exposure across Q1--Q5, so it is not a pure volatility sort.
\end{block}}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Why the CSI effect is bounded}
\footnotesize
\textbf{Dilution.} CSI exclusions are small in cap-weight terms, so only limited benchmark weight can move.\\[0.4mm]
\textbf{Classification noise.} False positives remain hard to separate from true positives, limiting how aggressively the filter can be used.
\end{block}
\vspace{0.4mm}
\begin{alertblock}{Verdict}
\footnotesize
LowVol cuts risk more when the low-volatility anomaly is favorable, but it falls below the benchmark OOS. CSI's risk reduction is smaller and more benchmark-like; its exclusions overlap the high-volatility tail, yet it keeps broad retained exposure.\\[0.4mm]
{\color{MainColour}\textbf{CSI complements low-volatility; it does not dominate it.}}
\end{alertblock}
\end{column}
\end{columns}
\sourcefoot{Test dynamic CSI Q5 excluded-name share is 70.4\%; OOS is 65.7\%. Retained exposure spans Q1--Q5.}
\end{frame}
