...
Directory <- file.path(Path, "06_Presentations/02_FinalPresentation/Necessary/FinalPresentation_June/Draft")
Directory_LaTeX <- file.path(Directory, "FinalPresentation_TristanLeiter_h11815352_Draft.Rnw")
setwd(Directory)
...
\node[pill] at (2.75,0) {Expanding-window CV};
\node[pill] at (5.5,0)  {4 feature sets};
\node[pill] at (8.25,0) {AP \& recall@FPR};
\end{tikzpicture}
\end{center}
...
\vspace{0.5em}
\textbf{\color{MainColour}Result:} AP $\approx 4.5$--$4.7\times$ the base rate; Test-AUC $\approx 0.88$; recall@FPR3 $\approx 0.20$.\\[6pt]
...
\end{block}
\remarkfoot{Best Test-AP: temporary 0.199, permanent 0.142. Metrics in Appendix A10 to A13.}
\end{frame}
...
\scriptsize
\textbf{The shown example is temporary CSI, \agexp{}.} The exclusion threshold is calibrated on the training set to ensure for no data leakage, then carried \emph{unchanged} to the held-out 2016--2019 test set.
\vspace{0.15em}
\begin{center}
...
\begin{frame}{Modelling IV: The Benefits and Drawbacks of VAE}
\scriptsize
...
\begin{itemize}
  ...
  \item \agexp{} retains slightly higher temporary OOS AUC: $0.8961$ vs. $0.8950$ compared to \agvae{}.
...
%==== 19. Modelling V: Summary and Implications ===============================%

\begin{frame}{Modelling V: Implications for index construction}
\centering
\vspace{0.7em}
...
\begin{tikzpicture}[x=1cm,y=1cm,
  ...
  \node[hero, fill=MainColour, text=white, text width=3.9cm, font=\scriptsize\bfseries] (take) at (6.1,4.05)
    {Index gains are bounded --- difficult to beat the benchmark's risk-return.};
...
\scriptsize
The model produces a probability of a CSI in the next year. A threshold turns this probability into a binary exclusion flag. The flag is then applied to market-cap weights. The two regimes differ only in how long the flag persists after it first emerges.
\end{alertblock}
...
\node[uni] at (0,0)   {\textbf{Total Market}\\[1pt]\tiny all eligible firms\\[-1pt]\tiny $\geq$ USD 100M};
...
\scriptsize
\begin{itemize}
  \item Prior-December market cap; minimum USD 100M.
...
\textbf{\color{MainColour}Test-set validation.} The CSI-methodology also works within the test-set. Permanent-CSI Risk/Volatility ratio rises from $0.89 \to \mathbf{1.00}$.\\[5pt]
\end{column}
\end{columns}

\begin{alertblock}{Summary}
\scriptsize
The crash-filter appears beneficial in both evaluation windows, but the evidence differs by split. The OOS result is a positive benchmark-relative return delta across the four universes. The test-set results are shown separately and serve as a comparison within a different market regime.
\end{alertblock}
...
\scriptsize
\textbf{Best strategy per universe net of 10 bps costs vs.\ themarket-weighted BM.}
\vspace{0.3em}
...
Universe & Strategy & Geo ret. & $\Delta$ pp & Risk/Vol. & Max DD \\
\midrule
...
% Total Risk/Vol. crosses 1.0). To show the Temporary winners instead, replace the
...
\scriptsize
In the different 2016-2019 market-regime a single consistent strategy (Exp. Dataset + VAE with Youden computed thresholds) improves Risk/Volatility Ratios and cuts drawdown in every universe, with Total Risk/Vol. crossing 1.0. The same small-cap tilt appears ($+0.83$pp Small vs.\ $+0.26$pp Large).
\end{block}
\end{frame}
...
\remarkfoot{Mean best benchmark-relative alpha across universes at 20\,bps.}
\vspace*{\fill}
\end{frame}

%==== Main Summary: Mechanism ================================================%
...
\begin{tikzpicture}[x=1cm,y=1cm,
  num/.style={circle, text=white, inner sep=0pt, minimum size=0.56cm, font=\scriptsize\bfseries}]

  % ---- spine flowing into the verdict ----
  \draw[-{Stealth[length=2.8mm]}, MainColour!55, line width=1.3pt] (0.55,5.95) -- (0.55,2.0);

  % ---- step 1 (capability) ----
  \node[num, fill=posgreenL] at (0.55,5.85) {1};
  \node[anchor=west, font=\small\bfseries, text width=9.4cm] at (1.05,5.99) {Models rank implosion risk well};
  \node[anchor=west, font=\scriptsize, text width=9.4cm] at (1.05,5.62) {AUC $\approx 0.88$; AP $\approx 4.5\times$ the base rate.};

  % ---- step 2 ----
  \node[num, fill=MainColour] at (0.55,4.85) {2};
  \node[anchor=west, font=\small\bfseries, text width=9.4cm] at (1.05,4.99) {But the exclusion cutoff is unavoidably noisy};
  \node[anchor=west, font=\scriptsize, text width=9.4cm] at (1.05,4.62) {False positives overlap true positives: $\approx$4 healthy firms flagged per real CSI.};

  % ---- step 3 ----
  \node[num, fill=MainColour] at (0.55,3.85) {3};
  \node[anchor=west, font=\small\bfseries, text width=9.4cm] at (1.05,3.99) {So the gain comes from reweighting, not avoidance};
  \node[anchor=west, font=\scriptsize, text width=9.4cm] at (1.05,3.62) {The false-positive cost is the main drag; retained-name reweighting carries the alpha.};

  % ---- step 4 ----
  \node[num, fill=MainColour] at (0.55,2.85) {4};
  \node[anchor=west, font=\small\bfseries, text width=9.4cm] at (1.05,2.99) {And the effect is dilution-bounded};
  \node[anchor=west, font=\scriptsize, text width=9.4cm] at (1.05,2.62) {CSI firms are $\approx$1\% of cap --- the gain is largest in Mid and Small Cap.};

  % ---- verdict banner ----
  \node[rounded corners=5pt, fill=MainColour, text=white, align=center,
        text width=10.1cm, inner sep=7pt] at (5.3,1.2)
    {\small\bfseries A bounded risk-reduction and portfolio-composition tool --- not a crash-avoidance alpha engine.\\[2pt]
     \scriptsize\mdseries Reliably cuts drawdown and modestly lifts Risk/Vol.: useful, principled, and honestly limited.};

\end{tikzpicture}

\remarkfoot{All results net of 10 bps; OOS 2020--2024. Attribution: portfolio-mechanism evidence, not causal proof of avoided crash events.}
\end{frame}
...
The data handling and model analysis was performed locally. Model training and index-construction
was implemented via Cloud Computing (Vast.ai and GCP).
...
\begin{frame}{Same universe, window, and costs: what does the CSI screen add?}
\scriptsize

\begin{alertblock}{Methodology: low-volatility comparator}
\scriptsize
\begin{itemize}\setlength\itemsep{1pt}
  \item Following the methodology by Baker et al. (2010), firms are sorted by trailing 5-year realized volatility into quintiles Q1 (low-vol)--Q5 (high-vol).
...
\footnotesize
The low-volatility screen removes firms because they are volatile. The CSI screen removes firms because the model ranks them as likely implosion candidates. The comparison asks whether that extra classification layer improves index outcomes without simply reproducing a generic volatility tilt.
\end{block}
...
% SLIDE 2 -- Results
% =====================================================================
\begin{frame}{Low-vol's edge is large, but conditional on the mark regime.}
  \centering
  \vspace{1mm}
  \begin{columns}[T,onlytextwidth]
    % ---- Risk/Vol. chart ----
    \begin{column}{0.5\textwidth}
      \centering
      {\footnotesize\color{MainColour} Risk/Vol. ratio}\\[1mm]
      \begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \end{columns}
    ...
    \tikz\fill[MainColour](0,0) rectangle (0.24,0.24);~CSI\par}
  \vspace{1mm}
  \begin{alertblock}{What it costs to be defensive (geo.\ return delta)}
    \footnotesize
    In the OOS-set, the Low-volatility portfolio performs considerably worse than in the test period ($-3.4$\,pp). The  CSI-model remains stable ($+0.3$\,pp). The maximum drawdowns of the low-volatility portfolio are smaller than those of the benchmark and the CSI-model.
  \end{alertblock}
