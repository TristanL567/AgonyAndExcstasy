...
\begin{tikzpicture}[font=\tiny, yscale=2.6, xscale=0.78]
  ...
  \foreach \x/\o/\t/\lbl in {0.35/0.43/0.41/Total, 2.15/0.15/0.24/Large,
                             3.95/0.51/0.33/Mid, 5.75/0.63/0.89/Small} {
...
\scriptsize
\textbf{Best strategy per universe vs.\ the market-weighted benchmark assuming 10 bps of transaction costs.}
\vspace{0.3em}
...
\midrule
Total & \agbase; Youden 3y & $13.3 \to 13.7$ & $+0.43$ & $0.60 \to \mathbf{0.64}$ & $-25.0 \to \mathbf{-23.8}$ \\
Large & \agbase; Youden 3y & $14.3 \to 14.4$ & $+0.15$ & $0.66 \to \mathbf{0.68}$ & $-24.8 \to \mathbf{-23.9}$ \\
Mid   & \agbase; FPR5 5y   & $9.5  \to 10.0$ & $+0.51$ & $0.41 \to \mathbf{0.42}$ & $-24.6 \to \mathbf{-24.5}$ \\
Small & \agvae; Youden 3y  & $7.6  \to 8.2$  & \textcolor{posgreen}{$\mathbf{+0.63}$} & $0.31 \to \mathbf{0.33}$ & $-29.1 \to -29.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
The risk-filter works better the smaller the market cap.: the alpha rises from $+0.15$pp (Large) to $+0.51$pp (Mid) to $+0.63$pp (Small). Smaller-cap universes include more firms in distress and exclusions move a larger part of the portfolio (about 85-106\% turnover vs.\ 10-13\% for the total market). Drawdown improves everywhere except for small caps.
\end{block}
\end{frame}
...
Universe & Strategy & Geo ret. & $\Delta$ pp & Sharpe & Max DD \\
\midrule
Total & \agvae; FPR5    & $13.3 \to 13.5$ & $+0.27$ & $0.60 \to \mathbf{0.61}$ & $-25.0 \to \mathbf{-24.7}$ \\
Large & \agvae; FPR5    & $14.3 \to 14.5$ & $+0.23$ & $0.66 \to \mathbf{0.67}$ & $-24.8 \to \mathbf{-24.5}$ \\
Mid   & \agbase; FPR5   & $9.5  \to 10.3$ & \textcolor{posgreen}{$\mathbf{+0.74}$} & $0.41 \to \mathbf{0.44}$ & $-24.6 \to \mathbf{-24.5}$ \\
Small & \aglatent; FPR3 & $7.6  \to 7.9$  & $+0.32$ & $0.31 \to \mathbf{0.32}$ & $-29.1 \to -29.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{block}{Main takeaway}
\scriptsize
The permanent filter tilts the gains toward Mid Cap ($+0.74$pp), its strongest universe. Direction matches the temporary track, but more exclusion over-removes firms with broad thresholds, which is why Youden is dropped in favour of FPR3 and FPR5.
\end{block}
\end{frame}
...
\midrule
Temp. & Total & \agbase; Youden 3y & $+0.43 / +0.43 / +0.42$ & \textcolor{posgreen}{unchanged} \\
Temp. & Large & \agbase; Youden 3y & $+0.15 / +0.15 / +0.15$ & \textcolor{posgreen}{unchanged} \\
Temp. & Mid   & \agbase; FPR5 5y   & $+0.51 / +0.51 / +0.51$ & \textcolor{posgreen}{unchanged} \\
Temp. & Small & \agvae; Youden 3y  & $+0.63 / +0.63 / +0.62$ & \textcolor{posgreen}{unchanged} \\
Perm. & Total & \agvae; FPR5       & $+0.27 / +0.27 / +0.27$ & \textcolor{posgreen}{unchanged} \\
Perm. & Large & \agvae; FPR5       & $+0.23 / +0.23 / +0.23$ & \textcolor{posgreen}{unchanged} \\
Perm. & Mid   & \agbase; FPR5      & $+0.74 / +0.74 / +0.74$ & \textcolor{posgreen}{unchanged} \\
Perm. & Small & \aglatent; FPR3    & $+0.32 / +0.32 / +0.32$ & \textcolor{posgreen}{unchanged} \\
\bottomrule
\end{tabular}
\end{table}
...
\end{block}
% TODO: this "active alpha" (annualized-return convention) does NOT equal the
% geometric Delta-pp on slides 21-22. Standardise deck-wide or keep the relabel.
\end{frame}
...
\textbf{20 bps evidence:} Youden has the strongest mean best alpha, $+0.35$pp.
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
\textbf{20 bps evidence:} FPR3 / FPR5 lead ($+0.35$ / $+0.33$pp); Youden averages $-0.20$pp.
\end{block}
\end{column}
\end{columns}
