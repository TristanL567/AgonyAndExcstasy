...
\midrule
Temp. & Total & AG Base Dataset & Youden / 3yr & 13.70\% & 13.29\% & +0.41pp \\
Temp. & Large & AG Base Dataset & Youden / 3yr & 14.40\% & 14.28\% & +0.12pp \\
Temp. & Mid & AG Base Dataset & FPR5 / 5yr & 9.93\% & 9.65\% & +0.28pp \\
Temp. & Small & AG Exp. Dataset + VAE & Youden / 3yr & 8.11\% & 7.65\% & +0.45pp \\
Perm. & Total & AG Exp. Dataset + VAE & FPR5 / perm. & 13.54\% & 13.29\% & +0.25pp \\
Perm. & Large & AG Exp. Dataset + VAE & FPR5 / perm. & 14.48\% & 14.28\% & +0.20pp \\
Perm. & Mid & AG Base Dataset & FPR5 / perm. & 10.16\% & 9.65\% & +0.51pp \\
Perm. & Small & AG Latent Dataset (VAE) & FPR3 / perm. & 7.82\% & 7.65\% & +0.16pp \\
\bottomrule
\end{tabular}
\end{table}
\scriptsize
VAE/non-raw variants beat raw in 5 of 8 track--index cases at 20 bps. The market-weighted benchmark is the zero-cost return reference and does not receive a strategy cost overlay.
\remarkfoot{Sources: presentation headline tables; AE-TC-RECHECK-002 zero-cost benchmark recomputation.}
\end{frame}
...
\midrule
Temp. & Total & AG Base Dataset & AG Base Dataset & +0.43pp & +0.41pp \\
Temp. & Large & AG Base Dataset & AG Base Dataset & +0.15pp & +0.12pp \\
Temp. & Mid & AG Base Dataset & AG Base Dataset & +0.51pp & +0.28pp \\
Temp. & Small & AG Exp. Dataset + VAE & AG Exp. Dataset + VAE & +0.64pp & +0.45pp \\
Perm. & Total & AG Exp. Dataset + VAE & AG Exp. Dataset + VAE & +0.27pp & +0.25pp \\
Perm. & Large & AG Exp. Dataset + VAE & AG Exp. Dataset + VAE & +0.23pp & +0.20pp \\
Perm. & Mid & AG Base Dataset & AG Base Dataset & +0.74pp & +0.51pp \\
Perm. & Small & AG Latent Dataset (VAE) & AG Latent Dataset (VAE) & +0.32pp & +0.16pp \\
\bottomrule
\end{tabular}
\end{table}
\scriptsize
Costs are applied at 0, 5, 10, and 20 bps. The 20 bps case is conservative enough for presentation, but not high enough to reverse the headline rankings. Active alpha uses the zero-cost benchmark convention.
\remarkfoot{Sources: transaction-cost robustness summary; AE-TC-RECHECK-002 zero-cost benchmark recomputation.}
\end{frame}
...
\remarkfoot{Sources: threshold-family and winner-turnover summaries; AE-TC-RECHECK-002 zero-cost formula.}
\end{frame}
